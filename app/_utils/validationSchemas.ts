...
export const clientSchema = z.object({
  ...
  dni: z
    .number({ message: "DNI must be a number!" })
    .positive({
      message: "DNI must be a positive number!",
    })
    .min(7, { message: "The minimum DNI of a client is 7" })
    .max(8, { message: "The maximum DNI allowed is 8" }),
  cuil: z
    .number({ message: "CUIL must be a number!" })
    .positive({
      message: "CUIL must be a positive number!",
    })
    .min(11, { message: "The minimum CUIL of a client is 11" })
    .max(11, { message: "The maximum CUIL allowed is 11" }),
});
